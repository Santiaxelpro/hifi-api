...
async def fetch_credentials():
    url = "https://api.github.com/gists/1c2ec972146bea4ded59cc03d88ce135"
    async with httpx.AsyncClient() as client:
        ...
        # Prefer Gist credentials first, then local ones.
        local_creds = [
            ("txNoH4kkV41MfH25", "dQjy0MinCEvxi1O4UmxvxWnDjt4cgHBPw8ll6nYBk98=")
        ]
        gist_hifi_creds = []
        gist_other_creds = []
        ...
        for key_entry in keys_data["keys"]:
            if key_entry.get("valid") == "True":
                ...
                if "hifi" in key_entry.get("formats", "").lower():
                    gist_hifi_creds.append(cred)
                else:
                    gist_other_creds.append(cred)
        
        # Order of attempts:
        # 1) Gist HiFi creds
        # 2) Gist non-HiFi creds
        # 3) Local creds
        hifi_creds = gist_hifi_creds
        other_creds = gist_other_creds + local_creds
...
async def main():
    ...
    # Keep order: Gist first, then local fallback.
